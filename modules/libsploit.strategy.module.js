...
function strategy_select() {
    ...
    if(osversion_between(3.12, 4.01)) {
    ...
    else if(osversion_between(13.0)) {
         //We firstly need to check if the client's device model is of one that this strategy supports
        ...
        if(supported) {
            ...
            puts('Chose Kuduma\'s jailbreakme');
            include('libsploit.ios.130'); //include the strategy module
            start_strategy('wk130go'); //schedule the strategy for launch
            return true;
...
function sploit_main() {
    ...
    try {
        ...
        if(!sploit_init()) {
            puts('Khai thác đang chạy.\.\.\.\.\.\.Vui lòng trượt đến khi được thông báo "SpringBoard"');
            return false;
        }
        if(!sploit_init()) {
